...
public class ReginePrimaSoluzione {
	...
	public List<Integer> cercaRegine(int N) {  //serve per avviare la ricorsione
		this.soluzione=null;
	...
	private boolean regine_ricorsiva(List<Integer> parziale, int livello, int N) {    //serve a gestire caso ricorsivo e caso terminale
		if(livello==N) {      //caso terminale
			this.soluzione=new ArrayList<Integer>(parziale);
			return false;    //non continuare la ricorsione
		}else {
